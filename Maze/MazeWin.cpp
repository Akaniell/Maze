	...
	for(int i = 0; i < height; i++){
		for(int j = 0; j < width; j++){
			switch(Maze_[i][j]){
				...
				case RIGHTWAY: SetConsoleTextAttribute(hConsole, FOREGROUND_RED | BACKGROUND_RED); cout<<"b "; break;
...
int main(){
	...
	MazeSet mazePoint;
	cout<<"¬ведите ширину лабиринта (число должно быть нечетным!): ";
	cin>>width;
	cout<<"¬ведите высоту лабиринта (число должно быть нечетным!): ";
	cin>>height;
